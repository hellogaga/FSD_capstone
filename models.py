...
if 'DATABASE_URL' in os.environ:
    ...
    database_path = "postgres://{}:{}@{}/{}".format(
        username, password, '127.0.0.1:5432', database_name)
...
def setup_db(app, database_path=database_path):
    ...
    the database_path will by default be provided by the above database_path
    for test purpose, another database path must be provided.
    '''
...
class Dictionary(db.Model):
    ...
    answer_record = db.relationship(
        'AnswerRecords', backref='Dictionary', lazy='dynamic')
...
class Categories(db.Model):
    ...
    Dictionary = db.relationship(
        'Dictionary', backref='Categories', lazy='dynamic')
...
class AnswerRecords(db.Model):
    ...
    dict_id = db.Column(db.Integer, db.ForeignKey(
    ...
    def __repr__(self):
        return (f'<User {self.user_id}, Question {self.dict_id},'
